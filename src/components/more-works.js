...
const Moreworks = ({ exclude, ...props }) => {
  ...
  const moreWorks = data.allContentfulWorks.nodes
    .filter(work => work.slug !== exclude)
    .slice(0, 1)
    ...
    <section className="block-works block-works-banner works-related">
      ...
      <div className="works">
        {moreWorks.map((work, index) => (
          <article className="work" style={{ backgroundColor: work.color }} key={index}>
            <Link to={`/${work.slug}/`}>
              <h3 className="work-title">{work.title}</h3>
              {work.project || work.timeline && (
